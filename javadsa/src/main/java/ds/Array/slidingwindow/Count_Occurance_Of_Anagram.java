...
public class Count_Occurance_Of_Anagram {
	...
	private static int countOccuranceOfAnagram(String text, String matcher) {
		...
		Arrays.asList(matcher.split("")).stream().forEach(x -> history.put(x, history.getOrDefault(x, 0) + 1));
